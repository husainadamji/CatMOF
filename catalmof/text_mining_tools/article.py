...
class Article:
    ...
    def read_paper(self):
        ...
        if os.path.exists(self.basepath + str(prefix) +
            ...
            os.remove("temp.html")
        elif os.path.exists(self.basepath + str(prefix) +
                          '/'+str(rest)+'.xml'):
            filename = (self.basepath + str(prefix) +
                        '/'+str(rest)+'.xml')
            replace(filename, ">\d+\w*[</\w+>]*</a>",
                    "></\w+></a>", "temp.xml")
            xml_doc = open("temp.xml", encoding="UTF-8")
            f = BeautifulSoup(xml_doc, 'xml')
            self.f = f
            self.original_f = f  # This is the original soup doc. Do not touch.
            os.remove("temp.xml")
        else:
    ...
    def get_authors(self):
        ...
        for i, tag in enumerate(meta_tags):
            if tag.has_attr('name'):
                if (tag['name'].lower() in ['citation_author', 'dc.creator', 'dc:creator']):
