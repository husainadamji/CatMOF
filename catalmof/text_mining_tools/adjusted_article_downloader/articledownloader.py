...
class ArticleDownloader:
  ...
  @traced
  def get_dois_from_search(self, query, rows=500, prefix=None, issn=None, mailto="null@null.com"):
    '''
    ...
    dois = []
    if prefix:
      print('------------- only querying '+str(prefix)+' ------------')
      base_url = 'https://api.crossref.org/prefixes/'+str(prefix)+'/works?query='
    else:
    ...
    if rows <= max_rows: #No multi-query needed
      search_url = base_url + query + '&rows=' + str(rows)+'&sort=score'+'&filter=from-pub-date:1998-01-01,type:journal-article,issn:'+str(issn)
  ...
  def get_html_from_doi(self, doi, writefile, mode):
    ...
    if mode == 'wiley':
      base_url = 'https://libproxy.mit.edu/login?url=http://onlinelibrary.wiley.com/doi/'
